...
export const TeachersAndDriverTable = () => {
    ...
    return (
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg bg-[#182237]">
            {isOpenModal && (
                <DriverAndTeacherModal isOpenModal={isOpenModal} setIsOpenModal={setIsOpenModal} />
            )}

            <div className="p-4 flex justify-between items-center">
                <Search
                    placeholder="Search for teachers..."
                    classname="border border-gray-700 outline-none focus-visible:outline-none px-2 py-0 focus-visible:ring-0 w-2/5"
                />
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {teachersData && teachersData.length > 0 ? (
                        teachersData.map((teacher: TeachersDriversProps) => (
                            <tr
                                key={teacher.busId}
                                className="dark:bg-gray-800 hover:bg-gray-900 cursor-pointer text-[var(--textSoft)] text-[0.7rem]"
                            >
                                <td className="px-6 py-4" rowSpan={8}>
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="">{teacher.username}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">{teacher.email}</td>
                                <td className="px-6 py-4">{teacher.phone}</td>
                                <td className="px-6 py-4">{teacher.busId}</td>
                                <td className="px-6 py-4">
                                    <div className="space-x-2">
                                        <Link href={`/dashboard/teachers/${teacher.busId}`}>
